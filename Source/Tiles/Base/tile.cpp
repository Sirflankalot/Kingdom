...
Tile::Tile(const Game& game, const sf::Vector2i& pos, const Ecosystem ecosystem, const sf::Color& light)
    : Sf_Entity(randomTexture(game)), m_tilePos(pos), m_ecosystem(ecosystem), m_light(light) {
...
void Tile::updateLight(const std::vector<Light>& lights) {
    ...
    for (const Light& light : lights) {
        if (m_light.r < 255 && m_light.g < 255 && m_light.b < 255) {
...
const sf::Texture& Tile::randomTexture(const Game& game) {
...
const bool Tile::isSwimmable() const {
...
const double Tile::getFriction() const {
    return m_info.friction;
}

const double Tile::getViscosity() const {
    return m_info.viscosity;
}

const Ecosystem Tile::getEcosystem() const {
    return m_ecosystem;
}

const unsigned Tile::getTxrId() const {
    return m_txrId;
}
