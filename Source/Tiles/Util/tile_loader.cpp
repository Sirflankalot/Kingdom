...
void Tile_Loader::readFile(std::ifstream& inFile) {
	...
	while (std::getline(inFile, line)) {
		if (wordFound(Viscosity, line))
			loadDouble(inFile, m_tile->viscosity);
		if (wordFound(Friction, line))
			loadDouble(inFile, m_tile->friction);

		if (wordFound(Walkable, line))
			loadBoolean(inFile, m_tile->isWalkable);
		if (wordFound(Swimmable, line))
			loadBoolean(inFile, m_tile->isSwimmable);

		if (wordFound(Colour, line))
			loadColour(inFile);
	}
}

const bool Tile_Loader::wordFound(std::string toFind, std::string line) {
...
void Tile_Loader::loadColour(std::ifstream& inFile) {
	...
	m_tile->colour = {(sf::Uint8) r, (sf::Uint8) g, (sf::Uint8) b, (sf::Uint8) a};
