...
const double Tile_Map::getFrictionAt(const sf::Vector2i& pos) {
    return at(pos)->getFriction();
}

const double Tile_Map::getViscosityAt(const sf::Vector2i& pos) {
    return at(pos)->getViscosity();
}

void Tile_Map::update(const float dt, const sf::Vector2i& playerTilePos, std::vector<Light>& lights) {
...
const bool Tile_Map::lightUpdateNeeded(const std::vector<Light>& lights) const {
    if (!lights.empty()) {
...
void Tile_Map::getScreenBounds(int& xStart, int& yStart, int& xEnd, int& yEnd,
    ...
    if (xEnd > Tiles::Info::MAP_SIZE - 1)
        xEnd = Tiles::Info::MAP_SIZE;
    if (yEnd > Tiles::Info::MAP_SIZE - 1)
        yEnd = Tiles::Info::MAP_SIZE;
}
