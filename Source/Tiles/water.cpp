...
namespace Tiles {
    ...
    Water::Water(Game& game, const sf::Vector2i& pos, Tile_Map& tileMap, const Ecosystem ecosystem,
        ...
        : Tile(game, pos, ecosystem, light), m_game(&game), m_tileMap(&tileMap) {
    ...
    void Water::update(const float dt) {
        if (!inWindowBounds(m_game->getWindow().get()))
            return;
    ...
    void Water::steppedOn() {
        m_tileMap->at(getTileMapPos()) =
            std::make_unique<Tiles::Ice>(*m_game, getTileMapPos(), m_tileMap, getEcosystem(), getLight());
    }
