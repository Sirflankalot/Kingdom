...
Sf_Entity::Sf_Entity(const sf::Texture& t) : m_sprite(t) {}
...
const bool Sf_Entity::inWindowBounds(const sf::RenderWindow& window) const {
    sf::Vector2f winPos = window.getView().getCenter();

    if (m_sprite.getPosition().x < winPos.x - Win_Info::WIDTH / 2 - m_sprite.getLocalBounds().width)
        return false;
    else if (m_sprite.getPosition().y < winPos.y - Win_Info::HEIGHT / 2 - m_sprite.getLocalBounds().height)
        return false;
    else if (m_sprite.getPosition().x > winPos.x + Win_Info::WIDTH / 2)
        return false;
    else if (m_sprite.getPosition().y > winPos.y + Win_Info::WIDTH / 2)
        return false;
