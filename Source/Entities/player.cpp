...
Player::Player(Game* game, Tile_Map* tiles)
    : Mob(game, game->getTexture(TXR_ENT_PLAYER), tiles), m_window(&game->getWindow().get()) {
    centerSpriteOrigin();
    m_view.setSize(Win_Info::WIDTH, Win_Info::HEIGHT);

    addComponent(std::make_unique<Component::Look_At_Mouse>(this, &game->getWindow().get()));

    addComponent(std::make_unique<Component::Steps_On_Tiles>(this, m_tileMap));

    addComponent(std::make_unique<Component::Ground_Friction>(this, m_tileMap));

    addComponent(std::make_unique<Component::Swimmable>(this, m_tileMap, true, *game));

    addComponent(std::make_unique<Component::Tile_Collidable>(this, m_tileMap));
