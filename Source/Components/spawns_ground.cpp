...
namespace Component {
    ...
    Spawns_Ground::Spawns_Ground(Mob* mob, Tile_Map* tiles, Player* player, const int maxDistFromPlayer,
                                 const int distToDespawn)
        : Component_Base(mob), m_player(player), m_tiles(tiles), m_distToDespawn(distToDespawn) {
    ...
    void Spawns_Ground::spawn(const int maxDistFromPlayer) {
        ...
        if (xBound >= Tiles::Info::MAP_SIZE)
            xBound = Tiles::Info::MAP_SIZE - 1;
        if (yBound >= Tiles::Info::MAP_SIZE)
            yBound = Tiles::Info::MAP_SIZE - 1;
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (m_tiles->at(xPos, yPos)->isWalkable()) {
                m_mob->setPosition({(float) xPos * Tiles::Info::SIZE, (float) yPos * Tiles::Info::SIZE});
                m_hasSpawned = true;
                break;
            }
        }
